...
class ProductController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('products-index')){
            ...
            return view('product.index', compact('all_permission','lims_warehouse_list'));
        }
        else
    ...
    public function productData(Request $request)
    {
        ...
        if(!empty($products))
        {
            foreach ($products as $key=>$product)
            {
                ...
                //dd($product_image);
                $nestedData['image'] = '<img src="'.url('public/images/product', $product_image).'" height="auto" width="80">';
                ...
                if($product->unit_id)
                    ...
                    $nestedData['unit'] = 'N/A';
                if(in_array("cost", $request['all_permission'])) {
                ...
                $nestedData['product'] = array( '[ "'.$product->type.'"', ' "'.$product->name.'"', ' "'.$product->code.'"',
                    ' "'.$nestedData['brand'].'"', ' "'.$nestedData['category'].'"', ' "'.$nestedData['unit'].'"', ' "'.$product->cost.'"',
                    ' "'.$product->price.'"', ' "'.$tax.'"', ' "'.$tax_method.'"', ' "'.$product->alert_quantity.'"', ' "'.preg_replace('/\s+/S', " ",
                        $product->product_details).'"', ' "'.$product->id.'"', ' "'.$product->product_list.'"', ' "'.$product->qty_list.'"',
                    ' "'.$product->price_list.'"', ' "'.$product->qty.'"', ' "'.$product->image.'"',' "'.$promotion_price.'"]'
                );
    ...
    public function product_detail($id)
    {
      ...
      //  dd($sale);
        $purchase = Purchase::orderBy('purchases.id', 'DESC')
           ...
           $lot_purchase  = DB::table('product_purchases')
                            ->join('purchases','product_purchases.purchase_id','=','purchases.id')
                            ->join('suppliers','purchases.supplier_id','=','suppliers.id')
                            ->where('product_purchases.product_id', '=', $id)
                            ->orderBy('product_purchases.product_id','DESC')
                            ->select('product_purchases.qty','product_purchases.total','product_purchases.created_at','suppliers.name','purchases.reference_no','purchases.status','purchases.payment_status','purchases.paid_amount')
                            ->get();
        ...
        // return $return_purchase; exit();
        return view('product.detail', compact('detail', 'lims_product_data', 'sale', 'purchase','lot_purchase', 'transfer', 'adjustment', 'tax', 'return', 'return_purchase'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $data['product_details_ar'] = str_replace('"', '@', $data['product_details_ar']);
        $data['ingredients'] = str_replace('"', '@', $data['ingredients']);
        $data['ingredients_ar'] = str_replace('"', '@', $data['ingredients_ar']);
        $data['how_to_use'] = str_replace('"', '@', $data['how_to_use']);
        $data['how_to_use_ar'] = str_replace('"', '@', $data['how_to_use_ar']);
...
//            ]);
            ...
            if(!isset($data['best_sale_featured'])){
                $data['best_sale_featured'] = 0;
            }else{
                $data['best_sale_featured'] = 1;
            }
            if(!isset($data['mini_shop_featured'])){
                $data['mini_shop_featured'] = 0;
            }else{
                $data['mini_shop_featured'] = 1;
            }
...
//            }
                ...
                foreach ($data['diff_price'] as $key => $diff_price) {
                    if($diff_price) {
                        ...
                        if($lims_product_warehouse_data) {
                            ...
                            $lims_product_warehouse_data->alert_qty = $data["diff_qty"][$key];
        ...
        public function show()
    {
     ...
     public function productBywarehousesearch(Request $request)
    {
        $data = $request->all();
     $warehouse_id = $data['warehouse_id'];
        ...
        $result = DB::table('products')
                ...
                ->select('products.*','product_warehouse.qty','brands.title','units.unit_name')
                ->get();
        ...
        if($role->hasPermissionTo('products-index')){
            ...
            if(empty($all_permission))
                $all_permission[] = 'dummy text';
            return view('product.warehousesearch', compact('all_permission','lims_warehouse_list','warehouse_id','result'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    ...
    public function get_product_barcode(Request $request)
    {
        ...
        for($i=0; $i<count($product_code) ;$i++){
            $lims_product_data[]= Product::where('code',$product_code[$i])->first();
            $barcode = DNS1D::getBarcodePNG($lims_product_data[$i]->code, $lims_product_data[$i]->barcode_symbology);
        }
        return view('product.product-barcode', compact('lims_product_data','product_code','qty','paper_size','barcode'));
