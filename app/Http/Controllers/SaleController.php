...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
            $totalData = Sale::where('warehouse_id', Auth::user()->warehouse_id)->count();
        }else {
        ...
        else
        {
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                ...
                $totalFiltered = Sale::
                            join('customers', 'sales.customer_id', '=', 'customers.id')
                            ->whereDate('sales.created_at', '=' , date('Y-m-d', strtotime(str_replace('/', '-', $search))))
                            ->where('sales.user_id', Auth::user()->warehouse_id)
                            ->orwhere([
                                ['sales.reference_no', 'LIKE', "%{$search}%"],
                                ['sales.warehouse_id',  Auth::user()->warehouse_id]
                            ])
            ...
            foreach ($sales as $key=>$sale)
            {
                ...
                if ($sale->document !=null) {
                    $nestedData['options'] .= '<li>
                        <a  target="_blank" href="' . url('/public/sale/documents', $sale->document) . '" class="btn btn-link"><i class="dripicons-document-remove"></i> ' . 'Attachment' . '</a>
                        </li>';
                }
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
          ...
          //  dd($lims_customer_list);
            ...
            return view('sale.create',compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list','lims_customer_group_all'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        $lims_quotation_data = Quotation::where('id',$data['quotation_id'])->first();
        ...
        if($data['pos']){
            ...
            if ($data['other_payment'] == null){
                $balance = $data['grand_total'] - $data['paid_amount'];
                $data['paid_amount'] = $request->paid_amount;
            }else{
                $balance = $data['grand_total'] - ($data['paid_amount'] + $data['other_payment']);
                $data['paid_amount'] = $data['paid_amount'] + $data['other_payment'];
            }
          ...
          //  $balance = $data['grand_total'] - $data['paid_amount'];
            ...
            if ($lims_quotation_data == null){
                $data['reference_no'] = 'sr-' . date("Ymd") . '-' . date("his");
            }else{
                $quo_reffer = $lims_quotation_data->reference_no;
                $quo_reffer_last = substr($quo_reffer, 8, 23);
                $data['reference_no'] = 'ecom-sr-' . $quo_reffer_last;
            }
        ...
        $data['sale_note'] = str_replace(["\r\n", "\r", "\n"], " , ", $sale_note);

        $lims_sale_data = Sale::create($data);
        $created_at =  date('Y-m-d', strtotime($lims_sale_data['created_at']));
        ...
        if($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            ...
            if($data['other_payment'] == null ){
                ...
                $lims_payment_data->amount = $data['paid_amount']-$data['other_payment'];
                $lims_payment_data->change = $data['paying_amount'] - ($data['paid_amount']-$data['other_payment']);
            }


            $lims_payment_data->paying_method = $paying_method;
            $lims_payment_data->payment_note = $data['payment_note'];
            $lims_payment_data->payment_date = $created_at;
            $lims_payment_data->created_at = $created_at;
            $lims_payment_data->save();

            if ($data['other_payment'] != null ){

                if($data['paid_by_id2'] == 1)
                    $paying_method = 'Cash';
                elseif ($data['paid_by_id2'] == 2){
                    $paying_method = 'Gift Card';
                }
                elseif ($data['paid_by_id2'] == 3)
                    $paying_method = 'Credit Card';
                elseif ($data['paid_by_id2'] == 4)
                    $paying_method = 'Cheque';
                elseif ($data['paid_by_id2'] == 5)
                    $paying_method = 'Paypal';
                elseif ($data['paid_by_id2'] == 7)
                    $paying_method = 'Bkash';
                else
                    $paying_method = 'Deposit';
                ...
                $data['payment_reference'] = 'spr-'.date("Ymd").'-'.date("his");
                $lims_payment_data->payment_reference = $data['payment_reference'];
                $lims_payment_data->amount = $data['other_payment'];
                $lims_payment_data->change = ($data['paying_amount'] - (($data['paid_amount']-$data['other_payment']) + $data['other_payment']));
                $lims_payment_data->paying_method = $paying_method;
                $lims_payment_data->payment_note = $data['payment_note'];
                $lims_payment_data->payment_date = $created_at;
                $lims_payment_data->created_at = $created_at;
            ...
            $lims_payment_data = Payment::where('paying_method',$paying_method)->orderBy('id','DESC')->first();
            ...
            elseif ($lims_payment_data->paying_method == 'Bkash') {
                PaymentWithBkash::create($data);
            }
    ...
    public function getCustomerAddress($id)
    {
        ...
        return $lims_customer_data->address;
    }
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        if(!$lims_product_data) {
            $lims_product_data = Product::join('product_variants', 'products.id', 'product_variants.product_id')
                ...
                ->select('products.*', 'product_variants.id as product_variant_id', 'product_variants.item_code', 'product_variants.additional_price','product_warehouse.price')
                ->where('product_variants.item_code', $product_code[0])
                ->first();
            $product_variant_id = $lims_product_data->product_variant_id;
        }

//        $warehouses = Product_Warehouse::where('product_id',$lims_product_data->id)->get();
//        foreach ($warehouses as $warehouse ){
//            $warehouse_id[] = $warehouse->warehouse_id;
//        }
//
//        $product_warehouse = Product_Warehouse::where('product_id',$lims_product_data->id)
...
//        }elseif($lims_product_data->promotion == null && $lims_product_data->is_diffPrice == 1 ){
//            $product[] = $product_warehouse->price;
//        }
//        else
//            $product[] = $lims_product_data->price;
        ...
        if($lims_product_data->promotion && $lims_product_data->promotion_price != null && $todayDate <= $lims_product_data->last_date){
            $product[] = $lims_product_data->promotion_price;
        }
        else
            $product[] = $lims_product_data->price;
        ...
        foreach ($lims_product_sale_data as $key => $product_sale_data) {
           ...
           // dd($product);
            ...
            if($unit_data){
                ...
                $unit = '';
            $product_sale[0][$key] = $brand->title .'-'. $product->name . ' [' . $product->code . ']' ;
            ...
            if ($product->promotion_price == null) {
                ...
                $product_sale[6][$key] =$product_sale_data->net_unit_price.  '<br> <span style="font-size: 10px;">( Original price : ' . $product->price . ') </span>';
            }else{
               
                 $product_sale[6][$key] =$product->promotion_price.'<br> <span style="font-size: 10px;">(Original price : ' . $product->price .') </span>';
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($product_id as $key => $pro_id) {
            ...
            elseif( $product_sale['variant_id'] === null && (in_array($pro_id, $old_product_id)) ) {
             ...
             $logs = new Log();
             $logs->sale_id = $lims_sale_data->id;
             $logs->referrence = $lims_sale_data->reference_no;
             $logs->amount = $lims_sale_data->grand_total;
             $logs->action = 'Update';
    ...
    public function addPayment(Request $request)
    {
       ...
       // dd($data);
        ...
        $lims_payment_data->paying_method = $paying_method;
        $lims_payment_data->payment_note = $data['payment_note'];
        $lims_payment_data->payment_date = $data['payment_date'];
        $lims_payment_data->created_at = $p_date;
        $lims_payment_data->save();
        $lims_sale_data->save();

        $lims_payment_data = Payment::orderBy('id','DESC')->first();
        $data['payment_id'] = $lims_payment_data->id;
        ...
        if($lims_customer_data->email){
            ...
            try{
                ...
                {
                    $message->to( $mail_data['email'] )->subject( 'Kosmetica - Your Order Information' );
                });
            }
            catch(\Exception $e){
    ...
    public function getPayment($id)
    {
        ...
        foreach ($lims_payment_list as $payment) {
            ...
            elseif($payment->paying_method == 'Bkash'){
                $lims_payment_bkash_data = PaymentWithBkash::where('payment_id',$payment->id)->first();
                $mobile_transaction[] = $lims_payment_bkash_data->mobile_transaction;
            }
            else{
    ...
    public function updatePayment(Request $request)
    {
        ...
        elseif($data['edit_paid_by_id'] == 7){
            if($lims_payment_data->paying_method == 'Bkash'){
                $lims_payment_cheque_data = PaymentWithBkash::where('payment_id', $data['payment_id'])->first();
                $lims_payment_cheque_data->mobile_transaction = $data['edit_mobile_transaction'];
                $lims_payment_cheque_data->save();
            }
            else{
                $lims_payment_data->paying_method = 'Bkash';
                $data['mobile_transaction'] = $data['edit_mobile_transaction'];
                PaymentWithBkash::create($data);
            }
        }
        ...
        //updating payment data
        $lims_payment_data->account_id = $data['account_id'];
        //$lims_payment_data->created_at = $data['edit_payment_date'];
        //$lims_payment_data->updated_at = $data['edit_payment_date'];
        $lims_payment_data->amount = $data['edit_amount'];
    ...
    //    dd($lims_sale_data);
        ...
        if($lims_customer_data->email){
            ...
            $mail_data['payment_method'] = $lims_payment_data->paying_method;
            $mail_data['grand_total'] = $lims_sale_data->grand_total;
            $mail_data['total_paid'] = $lims_sale_data->total_paid;
            $mail_data['paid_amount'] = $lims_payment_data->amount;
           // dd($mail_data);
            try{
                Mail::send( 'mail.payment_details', $mail_data, function( $message ) use ($mail_data)
    ...
    public function todaySale()
    {
        ...
        $data['bkash_payment'] = Payment::where([
            ['paying_method', 'Bkash']
        ])->whereDate('created_at', date("Y-m-d"))->sum('amount');
     ...
     public function paidBySelection(Request $request)
    {
      ...
      public function deliveredbyselection(Request $request)
    { 
           $sale_id = $request['saleIdArray'];
          foreach ($sale_id as $id) {
             // $lims_sale_data = Sale::find($id);
             ...
             $customer_sale = DB::table('sales')->join('customers', 'sales.customer_id', '=', 'customers.id')->where('sales.id', $id)->select('sales.*','customers.name', 'customers.address', 'customers.city', 'customers.country')->first();

                  $de = new Delivery();
                  $de->reference_no='dr-' . date("Ymd") . '-'. rand(000000,999999);
                  $de->sale_id=$customer_sale->id;
                  $de->user_id=$customer_sale->user_id;
                  $de->address=$customer_sale->address;
                  $de->delivered_by= 'Self' ;
                  $de->recieved_by='Self';
                  $de->file='';
                  $de->note='';
                  $de->status= 3 ;
        ...
        return redirect()->route('delivery.index')->with('message','Successfully Delivered to delivery list');
