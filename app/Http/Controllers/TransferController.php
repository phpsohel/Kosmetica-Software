...
class TransferController extends Controller
{
       ...
       // dd(Auth::user()->warehouse_id);
        if($role->hasPermissionTo('transfers-index')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own'){
                ...
                $lims_transfer_all = Transfer::with('fromWarehouse', 'toWarehouse', 'user')->orderBy('id', 'desc')->where('to_warehouse_id',Auth::user()->warehouse_id)->orWhere('from_warehouse_id',Auth::user()->warehouse_id)->get();
            }
            else {
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('transfers-add')){
          ...
          //  dd($warehouse_id = Auth::user()->warehouse_id);
           if($warehouse_id == null){
               $lims_warehouse_list = Warehouse::where('is_active', true)->get();
           }else{
               $lims_warehouse_list = Warehouse::where('is_active', true)->where('id',$warehouse_id)->get();
           }
    ...
    public function store(Request $request)
    {
        ...
        $lims_transfer_data = Transfer::orderBy('id','DESC')->first();
    ...
    public function update(Request $request, $id)
    {
       ...
       // dd($product_id);
        ...
        $logs = new Log();
        $logs->transfer_id = $lims_transfer_data->id;
        $logs->referrence = $lims_transfer_data->reference_no;
        $logs->amount = $lims_transfer_data->grand_total;
        $logs->action = 'Update';
    ...
    public function transfer_change_status($id)
    {
       $lims_transfer_data = Transfer::where('id',$id)->first();
        $lims_product_transfer_data = ProductTransfer::where('transfer_id', $id)->get();
            ...
            foreach ($product_id as $key => $pro_id) {
                ...
                if ($lims_transfer_data->status == 3) {
                    ...
                    if ($lims_product_to_warehouse_data) {
                        ...
                        $lims_product_warehouse_data->product_id = $pro_id;
                        $lims_product_warehouse_data->variant_id = $variant_id;
                        $lims_product_warehouse_data->warehouse_id = $lims_transfer_data->to_warehouse_id;
                        $lims_product_warehouse_data->qty = $quantity;
                        $lims_product_warehouse_data->save();
                    }
        ...
        return redirect()->back()->with('message', 'Transfer successfully Completed !');
