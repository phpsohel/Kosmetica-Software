...
class BrandController extends Controller
{
    ...
    {
        $lims_brands = Brand::where('is_active', true)->pluck('title', 'id');
        $lims_brand_all = Brand::where('is_active',true)->get();
        return view('brand.create', compact('lims_brand_all' ,'lims_brands'));
    }
    ...
    public function edit($id)
    {
        ...
        $lims_parent_data = Brand::where('id', $lims_brand_data['parent_id'])->first();
        if($lims_parent_data)
            $lims_brand_data['parent'] = $lims_parent_data['title'];
    ...
    public function update(Request $request, $id)
    {
        ...
        $lims_brand_data->title_ar = $request->title_ar;
        $lims_brand_data->notes = $request->notes;
        $parent_id = $request->parent_id;
        if ($parent_id != null){
            $lims_brand_data->parent_id = $parent_id;
        }else{
            $lims_brand_data->parent_id = null;
        }
        $image = $request->image;
        if ($image) {
            $ext = pathinfo($image->getClientOriginalName(), PATHINFO_EXTENSION);
