...
class PurchaseController extends Controller
{
            ...
            foreach ($purchases as $key=>$purchase)
            {
                ...
                if ($purchase->document !=null) {
                    $nestedData['options'] .= '<li>
                        <a target="_blank" href="' . url('/public/documents/purchase', $purchase->document) . '" class="btn btn-link"><i class="dripicons-document-remove"></i> ' . 'Attachment' . '</a>
                        </li>';
                }
    ...
    public function create()
    {
            ...
            $lims_role_data = Roles::find($role->id);
        ...
        if($role->hasPermissionTo('purchases-add')){
         ...
         return view('purchase.create', compact('lims_supplier_list', 'lims_warehouse_list', 'lims_tax_list', 'lims_product_list_without_variant', 'lims_product_list_with_variant','permissions'));
    ...
    public function store(Request $request)
    {
       ...
       //dd($ex_date);
        ...
        foreach ($product_id as $i => $id) {
            ...
            $product_purchase['ex_date'] = $ex_date[$i];
       ...
       // dd($lims_purchase_data);
        ...
        foreach ($product_data as $key => $product) {
            if($product['tax_method'] == 1){
                $net_unit_cost = ((float)$cost[$key] - (float) $discount[$key]);
                $product_tax = $net_unit_cost * ($tax[$key]['rate'] / 100) * $qty[$key];
                $total = ($net_unit_cost * $qty[$key]) + $product_tax;
            }
            elseif($product['tax_method'] == 2){
                $net_unit_cost = (100 / (100 + $tax[$key]['rate'])) * ((float)$cost[$key] - (float) $discount[$key]);
                $product_tax = ((float)$cost[$key] - (float)$discount[$key] - $net_unit_cost) * $qty[$key];
                $total = ((float)$cost[$key] - (float)$discount[$key]) * $qty[$key];
            }
            if($data['status'] == 1){
    ...
    public function update(Request $request, $id)
    {
       ...
       // dd($lims_purchase_data);
        ...
        $logs = new Log();
        $logs->purchase_id = $lims_purchase_data->id;
        $logs->referrence = $lims_purchase_data->reference_no;
        $logs->amount = $lims_purchase_data->grand_total;
        $logs->action = 'Update';
...
//            $product_id = $product->id;
//            $product_info = Product_Warehouse::where('product_id', 100)->get();
//        }
//            return view('stock.index', compact('product_info','products'));
//
//    }
    ...
    {
        $role = Role::find(Auth::user()->role_id);
        if( $role->hasPermissionTo('stock_list') ) {
