...
class ReportController extends Controller
{
     ...
     public function productExpiryAlert()
    {
            ...
            $lims_product_data = Product::select('name','code', 'image', 'qty', 'alert_quantity')->where('is_active', true)->whereColumn('alert_quantity', '>', 'qty')->get();
            return view('report.expirydate_alert', compact('lims_product_data'));
    ...
    public function product_report_brand_wise(Request $request)
    {
        $data = $request->all();
        // dd($data);
        $start_date = $data['start_date'];
        ...
        $product_name = [];
        $product_qty = [];
        $lims_product_all = Product::select('id', 'name', 'qty', 'is_variant')->where('brand_id', $brand_id)->where('is_active', true)->get();
        foreach ($lims_product_all as $product) {
            $lims_product_purchase_data = null;
            $variant_id_all = [];
            if($warehouse_id == 0) {
        ...
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        $brandlist = Brand::where('is_active', true)->get();
        return view('report.brand-product-report',compact('product_id', 'variant_id', 'product_name', 'product_qty', 'start_date', 'end_date', 'lims_warehouse_list', 'warehouse_id','brandlist','brand_id'));
    }


    public function product_report_expiry(Request $request)
    {
        $data = $request->all();
        ...
        $product_purchase= DB::table('product_purchases')
                          ->whereDate('ex_date','>=', $start_date)
                          ->whereDate('ex_date','<=', $end_date)
                          ->get();
            ...
            foreach ($product_purchase as $key => $value) {
             ...
             $data['purchases_data'] = DB::table('product_purchases')
                             ...
                             ->select('product_purchases.ex_date','product_purchases.qty','products.image','products.name','products.code','purchases.reference_no','purchases.warehouse_id')
                             ->get();

                             if ($data['purchases_data']) {
                               return view('report.expirydate_alert',$data);
                             }else{
                                return view('report.expirydate_alert');
                             }
    ...
    public function paymentReportByDate(Request $request)
    {
        ...
        $end_date = $data['end_date'];
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        $lims_payment_data = Payment::whereDate('payments.payment_date', '>=' , $start_date)
            ->whereDate('payments.created_at', '<=' , $end_date)
            ->get();
        return view('report.payment_report',compact('lims_payment_data', 'start_date', 'end_date','lims_warehouse_list'));
    }

    public function paymentReportByDateWarehouse(Request $request)
    {
        $data = $request->all();
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
      ...
      //  dd($payment_by);
        ...
        if ($type == 'sale'){
            if ($payment_by == 'all') {
                $lims_payment_data = Payment::join('sales','payments.sale_id','=','sales.id')
                    ->whereDate('payments.payment_date', '>=' , $start_date)
                    ->whereDate('payments.payment_date', '<=' , $end_date)
                ...
                $lims_payment_data = Payment::join('sales','payments.sale_id','=','sales.id')
                    ...
                    ->whereDate('payments.payment_date', '<=' , $end_date)
                    ->where('sales.warehouse_id',$warehouse_id)
                    ->where('payments.paying_method',$payment_by)
                    ->where('payments.purchase_id',null)
                    ->select('payments.*')
                    ->get();
            }
        ...
        return view('report.payment_report_warehouse',compact('lims_payment_data', 'start_date', 'end_date','lims_warehouse_list','warehouse_id','type','payment_by'));
    }
    ...
    public function userReport(Request $request)
    {
      ...
      //  dd(Auth::user()->id);
        ...
        $lims_log_data = Log::where('user_id', $user_id)->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
       // dd($lims_log_data);
...
//            ->select('purchases.created_at', 'purchases.reference_no', 'product_purchases.ex_date', 'product_purchases.qty', 'products.name')
...
//      //  dd($purchase);
    ...
    public function expire_date_filter(Request $request)
    {
        $purchase = Purchase::orderBy('purchases.id', 'DESC')
            ...
            ->select('purchases.created_at', 'purchases.reference_no', 'product_purchases.ex_date', 'product_purchases.qty', 'products.name')
            ->where('products.is_active', 1)
            ->where('product_purchases.ex_date', '>=', $request->start_date)
            ->where('product_purchases.ex_date', '<=', $request->end_date)
            ->get();
        $start_date = $request->start_date;
        $end_date = $request->end_date;
