...
class AdjustmentController extends Controller
{
    ...
    public function adjustmentByCsv()
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('adjustment')){
            $lims_warehouse_list = Warehouse::where('is_active', true)->get();
            return view('adjustment.import', compact( 'lims_warehouse_list'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function storeImportAdjustment(Request $request)
    {
         //dd($request->all());
        ...
        $lims_adjustment_data = Adjustment::orderBy('id','DESC')->first();
        ...
        foreach ($product_data as $key => $product) {
            ...
            $lims_adjustment_data->total_qty += $qty[$key];
            $lims_adjustment_data->item = $key + 1;
