...
class QuotationController extends Controller
{
        ...
        if($role->hasPermissionTo('quotes-index')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own'){
                // $lims_quotation_all = Quotation::with('biller', 'customer', 'supplier', 'user')->orderBy('id', 'desc')->where('user_id', Auth::id())->get();
                    $lims_quotation_all = Quotation::with('biller', 'customer', 'supplier', 'user')->orderBy('id', 'desc')->where('warehouse_id',Auth::user()->warehouse_id)->get();
           } else{
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('quotes-add')){
            ...
            return view('quotation.create', compact('lims_biller_list', 'lims_warehouse_list', 'lims_customer_list', 'lims_supplier_list', 'lims_tax_list','lims_customer_group_all'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
        ...
        foreach ($lims_product_quotation_data as $key => $product_quotation_data) {
            ...
            $product_quotation[5][$key] = $product_quotation_data->discount;
             if ($product->promotion_price == null) {
                ...
                $product_quotation[6][$key] =$product_quotation_data->net_unit_price;
            }else{
               
             $product_quotation[6][$key] =$product->promotion_price.  '   ( Original price : ' . $product->price . ' )';
            }
            $product_quotation[7][$key] = $product_quotation_data->total;
        }
    ...
    public function createSale($id)
    {
        ...
        return view('quotation.create_sale',compact('quotation_id','lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_quotation_data','lims_product_quotation_data', 'lims_pos_setting_data'));
    }
