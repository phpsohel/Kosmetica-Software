...
<html>
  ...
  <style>
      .daterangepicker.expanded .calendar, .daterangepicker .periods {
  ...
  <body onload="myFunction()">
            ...
            <ul id="side-main-menu" class="side-menu list-unstyled">                  
              <li><a href="{{url('/dashboard')}}"> <i class="dripicons-meter"></i><span>{{ __('file.dashboard') }}</span></a></li>
               <?php
                ...
                $stock_list = DB::table('permissions')->where('name', 'stock_list')->first();
                $stock_list_active = DB::table('role_has_permissions')->where([
                    ['permission_id', $stock_list->id],
                    ['role_id', $role->id]
                ])->first();
              ...
              ?>
              @if($category_permission_active || $index_permission_active || $print_barcode_active || $stock_count_active || $stock_list_active || $adjustment_active)
              <li><a href="#product" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-list"></i><span>{{__('file.product')}}</span><span></a>
                <ul id="product" class="collapse list-unstyled ">
                  {{--<li id="category-menu"><a href="{{ route('add-color') }}">Color</a></li>--}}
                  ...
                  @if($stock_list_active)
                   <li id="purchase-list-menu"><a href="{{route('stock.index')}}">Stock List</a></li>
                 @endif
              ...
              @endif
                ...
                {{--<li><a href="#stocklist" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-card"></i><span>Stock List</span></a>--}}
                    {{--<ul id="stocklist" class="collapse list-unstyled ">--}}
                        {{--<li id="purchase-list-menu"><a href="{{route('stock.index')}}">Stock </a></li>--}}
                    {{--</ul>--}}
                {{--</li>--}}
              ...
              @endif
               ...
               <li><a href="#website" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-user"></i><span>Website</span></a>
                <ul id="website" class="collapse list-unstyled ">
                  @if($user_index_permission_active)
                  <li id="user-list-menu"><a href="{{ route('add-blog') }}">Add Blog</a></li>
                  <li id="user-list-menu"><a href="{{ route('blog-list') }}">Blog List</a></li>
                  @endif
                  <li id="user-list-menu"><a href="{{ route('tag-list') }}">Tag List</a></li>
                  <li id="user-list-menu"><a href="{{ route('product-tag-list') }}"> Product Tag</a></li>
                  <li id="user-list-menu"><a href="{{ route('review.index') }}"> Reviews </a></li>
                  <li id="user-list-menu"><a href="{{ route('slider.index') }}"> Slider </a></li>
                </ul>
              </li>
              ...
              <?php
                ...
                $brand_product_report_active = DB::table('permissions')
                    ->join('role_has_permissions', 'permissions.id', '=', 'role_has_permissions.permission_id')
                    ->where([
                        ['permissions.name', 'brand-product-report'],
                        ['role_id', $role->id] ])->first();
                ...
                $due_report_active = DB::table('permissions')
                      ...
                      ->where([
                        ...
                        ['role_id', $role->id] ])->first();
              ?>
              @if($profit_loss_active || $best_seller_active || $warehouse_report_active || $warehouse_stock_report_active || $product_report_active || $brand_product_report_active || $daily_sale_active || $monthly_sale_active || $daily_purchase_active || $monthly_purchase_active || $purchase_report_active || $sale_report_active || $payment_report_active || $product_qty_alert_active || $user_report_active || $customer_report_active || $supplier_report_active || $due_report_active)
              <li><a href="#report" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-document-remove"></i><span>{{trans('file.Reports')}}</span></a>
                <ul id="report" class="collapse list-unstyled ">
                  ...
                  @endif
                     ...
                     @if($brand_product_report_active)
                      <li id="brand-product-report-menu">
                          {!! Form::open(['route' => 'brand.product.wise', 'method' => 'post', 'id' => 'brand-product-report-form']) !!}
                          <input type="hidden" name="start_date" value="1988-04-18" />
                          <input type="hidden" name="end_date" value="{{date('Y-m-d')}}" />
                          <input type="hidden" name="warehouse_id" value="0" />
                          <input type="hidden" name="brand_id" value="0" />
                          <a id="brand-report-link" href="">Brand Product Report</a>
                          {!! Form::close() !!}
                      </li>
                      @endif
              ...
              <li><a href="#setting" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-gear"></i><span>{{trans('file.settings')}}</span></a>
                ...
                <li id="warehouse-menu"><a href="{{route('hub.index')}}">Hubs</a></li>
                <li id="warehouse-menu"><a href="{{route('area.index')}}">Area</a></li>
        ...
        <nav class="navbar">
          <div class="container-fluid">
            <div class="navbar-holder d-flex align-items-center justify-content-between">
              ...
              <span class="brand-big" style="left:12%;"> <a href="{{url('/dashboard')}}">@if($general_setting->site_logo)<img src="{{url('public/logo', $general_setting->site_logo)}}" width="150"> </a>&nbsp;&nbsp;@endif<a href="{{url('/dashboard')}}">
    ...
    <script type="text/javascript">
      ...
      $("a#brand-report-link").click(function(e){
          e.preventDefault();
          $("#brand-product-report-form").submit();
      });
